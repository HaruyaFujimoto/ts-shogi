...
export class PieceMove {
  ...
  private getSquarePotisionForOneSquare(): SquarePosition[] {
    ...
    const specific_piece_can_move_area = PieceMove.OneSquareMoveArea[this.piece_type];
    const current_file = this.current_position.file;
    const current_rank = this.current_position.rank;
    // functions
    const piece_move_area_to_square_position = (number: PieceMoveArea): SquarePosition => {
    ...
    const remove_square_out_of_shogi_board =
      (square_position: SquarePosition) => square_position.is_in_shogi_board

    // result
    const square_position_list: SquarePosition[] = specific_piece_can_move_area.map(piece_move_area_to_square_position)
