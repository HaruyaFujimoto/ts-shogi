...
export class PieceMoveOnDiagram {
  // static methods for
  static get_piece_master_by_current_position_and_diagram(current_position: SquarePosition, diagram: Diagram) {
  ...
  static sliece_in_where_can_move_on_diagram_for_long_piece (square_position_list: SquarePosition[], current_position: SquarePosition, diagram: Diagram) {
    const master = this.get_piece_master_by_current_position_and_diagram(current_position, diagram);
  ...
  static filter_in_in_where_can_move_on_diagram_for_one_square_piece(square_position_list: SquarePosition[], diagram: Diagram): SquarePosition[] {
    const square_position_not_has_masters_piece = (square_position: SquarePosition) =>  {
      const {file, rank} = square_position;
      const piece = diagram.shogi_board[file][rank].piece;
      return ! (piece?.master == diagram.turn);
    };
    const result_list: SquarePosition[] = square_position_list.filter(square_position_not_has_masters_piece);
    return result_list;
  }
