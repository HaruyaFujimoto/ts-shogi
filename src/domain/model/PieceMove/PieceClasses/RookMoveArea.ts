...
export class RookMoveArea implements IPieceMoveArea {
  public get_square_positions_as_plain(
    ...
    const up_position_list = this._square_positions_by_direction(current_position, "u");
    const right_position_list = this._square_positions_by_direction(current_position, "r");
    const down_position_list = this._square_positions_by_direction(current_position, "d");
    const left_position_list = this._square_positions_by_direction(current_position, "l");
    // 各方向の移動範囲を結合する
  ...
  public get_square_positions_as_on_diagram(
    ...
    const up_position_list_width_diagram = PieceMoveOnDiagram.sliece_in_where_can_move_on_diagram_for_long_piece(up_position_list, current_position, diagram);
    const right_position_list_width_diagram = PieceMoveOnDiagram.sliece_in_where_can_move_on_diagram_for_long_piece(right_position_list, current_position, diagram);
    const down_position_list_width_diagram = PieceMoveOnDiagram.sliece_in_where_can_move_on_diagram_for_long_piece(down_position_list, current_position, diagram);
    const left_position_list_width_diagram = PieceMoveOnDiagram.sliece_in_where_can_move_on_diagram_for_long_piece(left_position_list, current_position, diagram);
    // 各方向の移動範囲を結合する
