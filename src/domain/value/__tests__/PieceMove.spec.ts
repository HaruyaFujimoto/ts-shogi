...
test("get PieceMove test,", () => {
  ...
  let move_area_as_list: number[][] = move_area.map(square_position => [square_position.file, square_position.rank]);
  let expect_list = [
    [2, 1],
    [2, 2],
    [1, 2],
  ];
  move_area_as_list.map(
    (file_rank_tuple, i) => {
  ...
  move_area_as_list = move_area.map(square_position => [square_position.file, square_position.rank]);
  expect_list = [
    [9, 8],
    [8, 8],
    [8, 9],
